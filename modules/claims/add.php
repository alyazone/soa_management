...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($vehicle_type_err) && empty($entries_err)){
        try {
            ...
            // Prepare an insert statement for the claim
                ...
                if($stmt->execute()){
                    ...
                    if (!empty($_FILES['receipt_files']['name'][0])) {
                        ...
                        for ($i = 0; $i < $fileCount; $i++) {
                            if ($_FILES['receipt_files']['error'][$i] === UPLOAD_ERR_OK) {
                                ...
                                if (!in_array($fileType, $allowedTypes) && 
                                    !in_array(strtolower($fileExtension), ['pdf', 'jpg', 'jpeg', 'png'])) {
                        ...
                        if (!empty($uploadedFiles)) {
                            $receiptSql = "INSERT INTO claim_receipts (claim_id, file_name, original_file_name, file_type, file_size) 
                                          VALUES (:claim_id, :file_name, :original_file_name, :file_type, :file_size)";
...
<div class="col-md-10 ml-sm-auto px-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Submit Reimbursement Claim</h1>
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" id="claimForm" enctype="multipart/form-data">
                <div class="row">
                    <div class="col-md-6">
                ...
                <!-- Receipt Uploads Section -->
                <h5 class="font-weight-bold mt-4">Receipt Uploads</h5>
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i> Upload receipts for your expenses. Supported file types: PDF, JPG, PNG. Maximum file size: 5MB per file.
                </div>
                ...
                <div class="card mb-4">
                    <div class="card-body">
                        <div class="form-group">
                            ...
                            <input type="file" class="form-control-file" id="receiptFiles" name="receipt_files[]" multiple accept=".pdf,.jpg,.jpeg,.png">
                            <small class="form-text text-muted">You can select multiple files at once.</small>
                        </div>
                        <div id="filePreviewContainer" class="mt-3 row">
    ...
    document.getElementById('receiptFiles').addEventListener('change', function(e) {
        ...
        if (this.files.length > 0) {
            for (let i = 0; i < this.files.length; i++) {
                ...
                let fileIcon = '<i class="fas fa-file-pdf fa-3x text-danger"></i>';
                if (fileType.includes('image')) {
                    fileIcon = '<i class="fas fa-file-image fa-3x text-primary"></i>';
                }
                ...
                previewCard.innerHTML = `
                    <div class="card">
                        <div class="card-body text-center">
                            ${fileIcon}
                            <h6 class="mt-2 text-truncate" title="${file.name}">${file.name}</h6>
